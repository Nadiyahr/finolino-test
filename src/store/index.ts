...
export default createStore<State>({
  ...
  actions: {
    ...
    APPLY_FILTERS: function ({ commit }, payload) {
      commit('SET_APPLAY', payload);
      commit('SORT_GOODS');
    },
  },
  mutations: {
    ...
    SORT_GOODS(state, payload = state.sortedGoods) {
      ...
      const sortBy = {
        Newest: () =>
          clone.sort(
            (prev, next) => Date.parse(next.update_date) - Date.parse(prev.update_date)
          ),
    ...
    SET_APPLAY(state, payload) {
      const priceFilter = state.byPrice.length ? [state.byPrice.join('-')] : [];
      state.filterTags = {
        bySize: state.bySize,
        bySeason: state.bySeason,
        byPrice: priceFilter,
      };

      let goods = [...payload] as Good[];
      ...
      const filtred = goods.filter(({ sizes, seasons }) => {
        const filterBy = (arr: string[], sizeArr: string[]) =>
          arr.some((el) => sizeArr.indexOf(el) >= 0);
      ...
      const fitredByPrice = filtred.filter(
        ({ price }) => Number(price) >= byPrice[0] && Number(price) <= byPrice[1]
      );
